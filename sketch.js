...
const BOOT_LINES = [
  'MONTIBEL OS  Version 1.0',
  'Copyright (C) 2024 montibel.',
  '',
  '  640K RAM OK',
  '',
  '  Iniciando MONTIBEL OS...',
];
...
function raised(x, y, w, h) {
  ...
  stroke(...C.gray);
  line(x+1, y+1, x+w-1, y+1); line(x+1, y+1, x+1, y+h-1);
  stroke(...C.darker);
  line(x+w, y, x+w, y+h); line(x, y+h, x+w, y+h);
  stroke(...C.darkGray);
  line(x+w-1, y+1, x+w-1, y+h-1); line(x+1, y+h-1, x+w-1, y+h-1);
}
...
class Win95Window {
  ...
  isActive() { return wins.length > 0 && wins[wins.length - 1] === this; }
  ...
  draw() {
    ...
    fill(...C.gray); noStroke();
    rect(this.x+2, this.y+2, this.w-4, this.h-4);

    const tx = this.x+this.B, ty = this.y+this.B, tw = this.w-this.B*2;

    if (this.isActive()) {
      gradientBar(tx, ty, tw, this.TB, color(...C.blue1), color(...C.blue2));
    } else {
      fill(...C.darkGray); noStroke(); rect(tx, ty, tw, this.TB);
    }

    fill(...C.white); noStroke();
    textFont('monospace'); textSize(11); textStyle(BOLD); textAlign(LEFT, CENTER);
  ...
  _drawBtns(tx, ty, tw) {
    ...
    fill(...C.black); textSize(9); textAlign(CENTER, CENTER);
    text('✕', closeX+S/2, by+S/2);

    this._drawBtn(maxX, by, S);
    noFill(); stroke(...C.black); strokeWeight(1);
    rect(maxX+3, by+4, S-6, S-7);

    this._drawBtn(minX, by, S);
    stroke(...C.black); strokeWeight(2);
  ...
  _drawBtn(x, y, s) {
    fill(...C.gray); noStroke(); rect(x, y, s, s);
    stroke(...C.white); line(x, y, x+s, y); line(x, y, x, y+s);
    stroke(...C.darker); line(x+s, y, x+s, y+s); line(x, y+s, x+s, y+s);
    stroke(...C.darkGray);
    line(x+s-1, y+1, x+s-1, y+s-1); line(x+1, y+s-1, x+s-1, y+s-1);
  }
...
class FotosWindow extends Win95Window {
  ...
  drawContent(x, y, w, h) {
    textFont('monospace'); textAlign(CENTER, TOP); noStroke();
    fill(...C.black); textSize(13); textStyle(BOLD);
    text('montibel', x+w/2, y+10);
    textStyle(NORMAL);
    fill(...C.darkGray); textSize(11);
    text('Fotografía análoga', x+w/2, y+30);

    const bw = 150, bh = 28;
    const bx = x+w/2-bw/2, by = y+h-bh-10;
    raised(bx, by, bw, bh);
    fill(...C.gray); noStroke(); rect(bx+2, by+2, bw-4, bh-4);
    fill(...C.black); textSize(12); textAlign(CENTER, CENTER);
...
class TextWindow extends Win95Window {
  ...
  drawContent(x, y, w, h) {
    fill(...C.black); noStroke();
    textFont('monospace'); textSize(12); textAlign(LEFT, TOP); textStyle(NORMAL);
...
class ContactoWindow extends Win95Window {
  ...
  drawContent(x, y, w, h) {
    fill(...C.black); noStroke();
    textFont('monospace'); textSize(12); textAlign(LEFT, CENTER);
...
const APP_DEFS = {
  ...
  proyectos: () => new TextWindow(200, 130, 320, 200, '💼 Proyectos', 'En construcción...'),
};
...
function drawLoading(f) {
  ...
  fill(...C.green); noStroke();
  textFont('monospace'); textSize(13); textAlign(CENTER, BOTTOM);
  text('Iniciando MONTIBEL OS...', width/2, by - 12);

  noFill(); stroke(...C.green); strokeWeight(1);
  rect(bx, by, bw, bh);
  fill(...C.green); noStroke();
...
function drawIcons() {
  for (let i = 0; i < ICONS.length; i++) {
    ...
    const ix = 14, iy = 14 + i*76;
    textAlign(CENTER, TOP); noStroke();
    textSize(28); fill(...C.white); text(emoji, ix+20, iy);
    textSize(11); text(label, ix+20, iy+32);
  }
}

// --- Taskbar ---
function drawTaskbar() {
  const H = 28, Y = height - H;
  fill(...C.gray); noStroke(); rect(0, Y, width, H);
  stroke(...C.white); strokeWeight(1); line(0, Y, width, Y);

  raisedBlock(2, Y+2, 56, H-4);
  fill(...C.black); textSize(12); textAlign(CENTER, CENTER);
  textStyle(BOLD); text('▶ Inicio', 30, Y+H/2); textStyle(NORMAL);

  const t = new Date();
  const ts = nf(t.getHours(), 2) + ':' + nf(t.getMinutes(), 2);
  const cW = 48, cX = width-cW-4;
  sunkenBlock(cX, Y+3, cW, H-6);
  fill(...C.black); textSize(11); textAlign(CENTER, CENTER);
  ...
  for (const w of wins) {
    ...
    fill(...C.black); textSize(11); textAlign(LEFT, CENTER);
...
function raisedBlock(x, y, w, h) {
  fill(...C.gray); noStroke(); rect(x, y, w, h);
  stroke(...C.white); line(x, y, x+w, y); line(x, y, x, y+h);
  stroke(...C.darker); line(x+w, y, x+w, y+h); line(x, y+h, x+w, y+h);
  stroke(...C.darkGray);
  line(x+w-1, y+1, x+w-1, y+h-1); line(x+1, y+h-1, x+w-1, y+h-1);
}
...
function sunkenBlock(x, y, w, h) {
  fill(...C.gray); noStroke(); rect(x, y, w, h);
  stroke(...C.darker); line(x, y, x+w, y); line(x, y, x, y+h);
  stroke(...C.white); line(x+w, y, x+w, y+h); line(x, y+h, x+w, y+h);
}

// --- Start Menu ---
function drawStartMenu() {
  const mW = 180, mH = 220;
  const mX = 2, mY = height-28-mH;
  raisedBlock(mX, mY, mW, mH);

  fill(...C.blue1); noStroke();
  rect(mX+2, mY+2, 22, mH-4);

  push();
  fill(...C.white); textSize(12); textAlign(CENTER, CENTER); textStyle(BOLD);
  translate(mX+13, mY+mH/2+20); rotate(-HALF_PI);
  text('MONTIBEL OS', 0, 0);
  textStyle(NORMAL);
  pop();

  const items = ['📁  Programas','📄  Documentos','⚙  Configuración',
                 '🔍  Buscar','❓  Ayuda','---','⏻  Apagar...'];
  for (let i = 0; i < items.length; i++) {
    ...
    if (mouseX > mX+24 && mouseX < mX+mW && mouseY > iy && mouseY < iy+28) {
      fill(...C.blue1); noStroke(); rect(mX+24, iy, mW-26, 28);
      fill(...C.white);
    } else { fill(...C.black); }
...
function mousePressed() {
  if (appState !== 'desktop') { goTo('desktop'); return; }

  const tbY = height-28;

  if (mouseX >= 2 && mouseX <= 58 && mouseY >= tbY) {
    startOpen = !startOpen; return;
  }
  if (startOpen) { startOpen = false; return; }

  if (mouseY >= tbY) {
    for (const w of wins) {
      if (mouseX >= w._taskX && mouseX <= w._taskX+110) {
        const isTop = wins[wins.length-1] === w;
        if (w.minimized)    { w.minimized = false; bringToFront(w); }
        else if (isTop)     { w.minimized = true; }
        else                { bringToFront(w); }
        return;
      }
    }
  ...
  for (let i = wins.length-1; i >= 0; i--) {
    ...
    if (w.onClose(mouseX, mouseY)) { wins.splice(wins.indexOf(w), 1); return; }
    if (w.onMin(mouseX, mouseY))   { w.minimized = true; return; }

    if (w instanceof FotosWindow && w.onGalBtn(mouseX, mouseY)) {
      window.open('fotos/', '_blank'); return;
    }
    if (w instanceof ContactoWindow && w.onMailBtn(mouseX, mouseY)) {
      window.open('mailto:montibel@gmail.com'); return;
    }

    if (w.onTitleBar(mouseX, mouseY)) {
      dragging = w; dragOX = mouseX-w.x; dragOY = mouseY-w.y;
    }
    return;
  }
